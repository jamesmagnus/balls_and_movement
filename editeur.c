...
{
	Map *pMap = NULL;
	int continuer=true, temps=0, tempsAncien=0, diaPrisRetour=AUCUN_BONUS;
	...
	while(continuer)
	{
		...
		{
			AffichageEditeur(pMoteurRendu, images, pMap, entrees, diaPrisRetour);
			diaPrisRetour = MiseAJourMap(pMap, images, &entrees, pMoteurSon, pSons);
			tempsAncien = temps;
		}
...
{
	int i=0, j=0, k=0, angleCurseur=335;               //Variable de comptage
	...
	images[MISSILE].position[10].x = images[GEMMES].position[0].x = images[CURSEUR].position[0].x = entrees.souris.position.x;
	images[MISSILE].position[10].y = images[GEMMES].position[0].y = images[CURSEUR].position[0].y = entrees.souris.position.y;

	SDL_RenderCopyEx(pMoteurRendu, images[CURSEUR].pTextures[0], NULL, &images[CURSEUR].position[0], angleCurseur, NULL, SDL_FLIP_NONE);

	if(diaPris != AUCUN_BONUS && diaPris != 100 && diaPris != 101)
	{
		SDL_RenderCopy(pMoteurRendu, images[GEMMES].pTextures[0], &images[GEMMES].position[diaPris], &images[GEMMES].position[0]);
	}
	else if (diaPris == 100)
	{
...
int MiseAJourMap (Map *pMap, sprite images[], ClavierSouris *pEntrees, FMOD_SYSTEM *pMoteurSon, Sons *pSons)
{
	...
	static int deplacement= -1, diaPris=AUCUN_BONUS, missile=0;
	int i, j, k=0;
	static int retour=AUCUN_BONUS;
	...
	{
		if(pEntrees->souris.touches[C_GAUCHE] && !pEntrees->clavier[SHIFT] && !diaPris && !missile)
		{
			if(BSons)
			{
				FMOD_System_PlaySound(pMoteurSon, S_CLICK+10, pSons->bruits[S_CLICK], false, NULL);
		...
		else if (diaPris && pEntrees->souris.touches[C_GAUCHE])
		{
			if(BSons)
			{
				FMOD_System_PlaySound(pMoteurSon, S_CLICK+10, pSons->bruits[S_CLICK], false, NULL);
			...
			pMap->planObjets[pos.x /TailleBloc][pos.y /TailleBloc] = diaPris;
			retour = diaPris = AUCUN_BONUS;
	...
	if(pEntrees->souris.touches[C_GAUCHE] && pEntrees->souris.position.x > images[AJOUTER_MISSILE_H].position[0].x && pEntrees->souris.position.x < images[AJOUTER_MISSILE_H].position[0].x + images[AJOUTER_MISSILE_H].position[0].w)
	{
	...
	if(pEntrees->souris.touches[C_GAUCHE] && pEntrees->souris.position.x > images[AJOUTER_MISSILE_V].position[0].x && pEntrees->souris.position.x < images[AJOUTER_MISSILE_V].position[0].x + images[AJOUTER_MISSILE_V].position[0].w)
	{
	...
	if (missile && pEntrees->souris.touches[C_GAUCHE])
	{
		switch(missile)
		{
		case 100:
